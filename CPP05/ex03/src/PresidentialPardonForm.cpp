...
PresidentialPardonForm::PresidentialPardonForm(std::string target): _target(target), AForm("Default_PresidentialPardonForm", 150, 150)			// 1 custom costructor
{
	std::cout << "PresidentialPardonForm document with custom values creation." << std::endl;
}

PresidentialPardonForm::PresidentialPardonForm(std::string target, int gradeToSign, int gradeToExecute):_target(target), AForm("PresidentialPardonForm", gradeToSign, gradeToExecute)	// 2 custom costructor
{
